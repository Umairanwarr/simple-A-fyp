...
export default function StoreDashboard() {
	...
	const storeName = useMemo(() => {
		try {
			const rawStore = localStorage.getItem('medicalStore');
			...
			return parsedStore?.name || 'Medical Store';
		} catch (error) {
			return 'Medical Store';
		}
	}, []);
			...
			<div className="max-w-[980px] mx-auto bg-white border border-gray-100 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 sm:p-10">
				...
				<div className="mt-8 flex flex-wrap gap-3">
					...
					<button
						type="button"
						onClick={() => {
							localStorage.removeItem('medicalStoreToken');
							localStorage.removeItem('medicalStore');
							navigate('/signin');
						}}
