        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
          <div className="flex items-center gap-4">
            <div className="w-[86px] h-[86px] rounded-full border-4 border-white overflow-hidden bg-[#E5F8F6] shadow-sm shrink-0">
              <img src={avatarUrl} alt={fullName || 'Patient'} className="w-full h-full object-cover" />
            </div>

            <div>
              <div className="flex items-center flex-wrap gap-2">
                <h3 className="text-[20px] font-bold text-[#1F2432]">{fullName || 'Patient'}</h3>
                <span
              ...
              <p className="text-[13px] text-[#6B7280] font-medium mt-1">{email || 'Patient Account'}</p>
              {!isProfileComplete && (
