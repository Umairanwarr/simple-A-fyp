...
import { fetchDoctorReviews } from '../../../services/authApi';
...
export default function ReviewsPage() {
  ...
  useEffect(() => {
    ...
    const loadDoctorReviews = async () => {
      ...
      try {
        setIsLoading(true);
        const data = await fetchDoctorReviews(doctorToken);
    ...
    window.addEventListener('doctor-reviews-updated', refreshHandler);

    return () => {
      isMounted = false;
      window.removeEventListener('doctor-reviews-updated', refreshHandler);
    };
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 sm:p-7 rounded-[30px] border border-gray-100 shadow-sm">
        <div className="mb-5">
          ...
          <p className="text-[14px] text-[#9ca3af] mt-1">See what patients said after completed appointments.</p>
        </div>
